...
import { deleteBeat } from '../../../../../src/services/tools/beats/delete';
...
describe('deleteBeat', () => {
  ...
  it('returns error when id is missing', async () => {
    const result = await deleteBeat(logger, {});
  ...
  it('returns error when beat is not found', async () => {
    ...
    const result = await deleteBeat(logger, { id: 'no-such-id' });
    expect(result.success).toBe(false);
  ...
  it('returns success when beat is deleted', async () => {
    ...
    const result = await deleteBeat(logger, { id: 'hb-1' });
  ...
  it('calls repo.deleteById with the provided id', async () => {
    mockRepo.deleteById.mockReturnValue(true);
    await deleteBeat(logger, { id: 'hb-42' });
