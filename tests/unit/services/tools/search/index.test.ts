...
describe('search_engine tool', () => {
  ...
  it('should honor num and start and compute positions', async () => {
    ...
    const result = await executeSearch(mockLogger, { query: 'test query', num: 3, start: 10 });
  ...
  it('should clamp num to the 1-100 range', async () => {
    ...
    await executeSearch(mockLogger, { query: 'test query', num: 500 });
    expect(serpapi.getJson).toHaveBeenCalledWith(expect.objectContaining({ num: 100 }));

    await executeSearch(mockLogger, { query: 'test query', num: 0 });
  ...
  it('should forward gl, hl, time_period and search_type to SerpAPI', async () => {
    ...
    await executeSearch(mockLogger, {
      query: 'test query',
      gl: 'br',
      hl: 'pt-br',
  ...
  it('should not include optional params when not provided', async () => {
    ...
    await executeSearch(mockLogger, { query: 'test query' });
  ...
  it('should reject an invalid time_period by ignoring it', async () => {
    ...
    await executeSearch(mockLogger, { query: 'test query', time_period: 'last_decade' });
  ...
  it('should reject an invalid search_type', async () => {
    const result = await executeSearch(mockLogger, { query: 'test query', search_type: 'shopping' });
  ...
  it('should return news results from news_results when search_type is news', async () => {
    ...
    const result = await executeSearch(mockLogger, { query: 'Estreito de Hormuz', search_type: 'news' });
  ...
  it('should default news searches to past_day when no time_period given', async () => {
    ...
    await executeSearch(mockLogger, { query: 'test query', search_type: 'news' });
  ...
  it('should map source object and source.authors on news results', async () => {
    ...
    const result = await executeSearch(mockLogger, { query: 'test query', search_type: 'news' });
