...
import { executeSearchViaSearxng } from '../../../../../src/services/tools/search/searxng';
...
describe('search_engine tool (SearXNG provider)', () => {
  ...
  it('honors num and approximates start as a page number', async () => {
    ...
    const result = await executeSearchViaSearxng(mockLogger, { query: 'test query', num: 3, start: 20 });
  ...
  it('clamps num to the 1-50 range', async () => {
    ...
    const over = await executeSearchViaSearxng(mockLogger, { query: 'test query', num: 500 });
    expect(JSON.parse(over.result as string)).toHaveLength(50);

    const under = await executeSearchViaSearxng(mockLogger, { query: 'test query', num: 0 });
  ...
  it('folds gl and hl into a single language param', async () => {
    ...
    await executeSearchViaSearxng(mockLogger, { query: 'test query', gl: 'br', hl: 'pt' });
  ...
  it('uses hl alone when gl is not provided', async () => {
    ...
    await executeSearchViaSearxng(mockLogger, { query: 'test query', hl: 'en' });
  ...
  it('does not double up the region when hl already carries one (regression: pt-br + br -> pt-br, not pt-br-BR)', async () => {
    ...
    await executeSearchViaSearxng(mockLogger, { query: 'test query', hl: 'pt-br', gl: 'br' });
  ...
  it('maps time_period to time_range, falling back past_hour to day', async () => {
    ...
    await executeSearchViaSearxng(mockLogger, { query: 'test query', time_period: 'past_week' });
    expect(fetchMock.mock.calls[0][0]).toContain('time_range=week');

    await executeSearchViaSearxng(mockLogger, { query: 'test query', time_period: 'past_hour' });
  ...
  it('maps search_type to categories and rejects invalid values', async () => {
    ...
    await executeSearchViaSearxng(mockLogger, { query: 'test query', search_type: 'news' });
    expect(fetchMock.mock.calls[0][0]).toContain('categories=news');
    expect(fetchMock.mock.calls[0][0]).toContain('time_range=day');

    await executeSearchViaSearxng(mockLogger, { query: 'test query', search_type: 'video' });
    expect(fetchMock.mock.calls[1][0]).toContain('categories=videos');

    const result = await executeSearchViaSearxng(mockLogger, { query: 'test query', search_type: 'shopping' });
  ...
  it('returns error when query is missing', async () => {
    const result = await executeSearchViaSearxng(mockLogger, {});
    expect(result.success).toBe(false);
  });
