...
- `landing/next.config.ts` sets `basePath: '/koris'` for the GitHub Pages project URL (`https://guilhermesalviano.github.io/koris`) and `images.unoptimized: true` (required for static export). Build output always lands in `landing/out/` (Next forbids `distDir` escaping the project directory), kept separate from `dist/` and `dist-web/`.
