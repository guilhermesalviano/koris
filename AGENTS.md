...
- GitHub Pages project URL (`https://guilhermesalviano.github.io`) and `images.unoptimized: true` (required for static export). Build output always lands in `landing/out/` (Next forbids `distDir` escaping the project directory), kept separate from `dist/` and `dist-web/`.
