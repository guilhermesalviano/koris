...
- `web/src/lib/chat-context.tsx` — `ChatProvider`/`useChat` hold conversation state, hydrate prior history from `/api/admin/chat/history`, stream replies, and poll server health every 5s. Chat is scoped to the `web` session (`agent.handle(message, 'web', ...)`, `src/dashboard/index.ts:102`).
