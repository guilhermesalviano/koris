...
/**
 * Locates a channel plugin's own directory so its `config.yml` can be found
 * at runtime even from a compiled `dist/` build — `tsc` only compiles `.ts`
 * files, so `config.yml` never gets copied next to the compiled `.js`, and
 * `__dirname` alone (pointing at `dist/plugins/channels/<name>`) would never
...
export function resolvePluginDir(pluginName: string, options: ResolvePluginDirOptions = {}): string {
  ...
  const cwdCandidates = [
    join(cwd, 'plugins', 'channels', pluginName),
    join(cwd, 'apps', 'client', 'plugins', 'channels', pluginName),
