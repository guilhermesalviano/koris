...
This repo already has a detailed **AGENTS.md** at the repo root — read it first. It covers the full architecture (message flow, AI providers, tools, plugins/skills, DB schema, web frontend, landing page) and conventions. This file only adds the commands quick-reference and a couple of things AGENTS.md doesn't spell out (single-test invocation, an rtk-prefix instruction found in `.github/copilot-instructions.md`).
...
- Type-check: `pnpm lint` (server), `pnpm lint:client` (web/), `pnpm lint:landing` (landing/) — must pass, strict TS (`noUnusedLocals`, `noUnusedParameters`, `noImplicitReturns`)
...
- Landing page: `pnpm landing:dev`, `pnpm landing:build`, `pnpm landing:preview`
